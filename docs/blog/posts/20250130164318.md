...
Below is a **Python script** that automates the setup of the ROS 1-ROS 2 bridge every time you need it. The script ensures that:

- **ROS 1 Noetic and ROS 2 Foxy** are properly sourced.
- The **ros1_bridge** is installed and launched.
- It verifies the bridge and the `/cmd_vel` topic.
- You can control the RoboClaw motor controller from ROS 2.

---

### **Python Script (`setup_ros_bridge.py`)**
```python
import os
import subprocess
import time
...
def send_velocity_command():
    """ Sends a test velocity command from ROS 2 """
    print("[✔] Sending velocity command from ROS 2...")
    os.system("ros2 topic pub /cmd_vel geometry_msgs/msg/Twist \"{linear: {x: 0.5}, angular: {z: 0.1}}\"")
...
1. **Save the script** as `setup_ros_bridge.py`
2. **Make it executable**:
   ```bash
   chmod +x setup_ros_bridge.py
   ```
3. **Run the script**:
   ```bash
